...
| `predict_surf_count.py` | Phase 3: pulls live forward-looking predictors (today+tomorrow, no auth token needed) via `get_surf_predictors.py`'s `build_predictor_map()`, trains production GBT quantile models (10th/50th/90th) on all of `training_features.csv`, and outputs a prediction with an 80% range for one or more times. Point estimate is the median model, not a separate mean-based one — guarantees it always falls inside its own range, which isn't true for a mean-based estimate on this right-skewed target (see `PROJECT_HISTORY.md`'s 2026-08-28 entry). |
