...
| `train_model.py` | Rebuilds the surfer-detection YOLOv8 model from a CVAT COCO export: tiles images/annotations to match production inference tiling exactly (imports `TILE_W`/`TILE_H`/`STEP_X`/`NUM_TILES` directly from `detect_surfers.py` so the two can't drift apart), converts to YOLO format, fine-tunes from a COCO-pretrained checkpoint, and validates on the held-out val split with real, printed precision/recall/mAP. Added 2026-08-29 to replace `detect_surfers_v2.ipynb` as the retraining path — rerun this (not the notebook) whenever there's a new or larger CVAT export. Each run writes to fresh, dated output folders (`data/cvat_out_coco/splits_tiled_<run-name>/`, `data/cvat_out_yolo_rebuilt_<run-name>/`, `data/model_out/<run-name>/`) rather than overwriting the current production model — nothing production-facing changes until `MODEL_PATH` is manually pointed at the new weights. `data.yaml`'s `nc`/`names` are derived from the COCO export's actual `categories` list (added 2026-09-05, was hardcoded to `nc: 1`/`names: ['surfer']`) — supports a future multi-class (e.g. posture) export automatically, no code change needed. See its own docstring (`python code/train_model.py --help`) for usage. |
| `select_labeling_candidates.py` | Phase 1 of the training-data-expansion plan (2026-09-05): produces a prioritized candidate-image CSV for manual CVAT labeling, combining real human-vs-model error cases (`data/reviews/model_spotcheck_50/review_counts.csv`, capped to the top N by a count-weighted score) with gap-fill sampling from `data/training_features.csv` (oversampling underrepresented months/tide ranges/crowd sizes/weather relative to the existing CVAT set) — every candidate filename is verified to exist on disk before inclusion. Output is a starting shortlist for review, not an auto-decision. Writes `analysis/training_data_expansion/labeling_candidates.csv` (gitignored). |
| `prepare_cvat_reimport.py` | Phase 2 support (2026-09-05): the original CVAT task for the existing 57 labeled images no longer exists, so adding the new posture attribute needs a fresh CVAT task. Merges `data/cvat_out_coco/splits/{train,val,test}` (a verified clean, non-overlapping partition — image ids 1-57, annotation ids 1-1451, zero collisions) back into one flat COCO 1.0 export matching CVAT's own export layout (`annotations/instances_default.json` + `images/default/`), zipped for direct upload/import. Writes `data/cvat_out_coco/reimport_existing_57.zip` (gitignored) and prints the CVAT import/attribute-setup/export steps. |
...
| `detector_training_metrics/` | Dark-themed remake of Ultralytics' own `results.png` for the production detector's training run, using the real per-epoch log already sitting at `data/model_out/20251013/train/runs/detect/train13/results.csv` (60 epochs) — no re-training, just restyling real numbers to match the site's theme. `plot_training_metrics.py` writes `detector_training_metrics.png` (git-tracked, embedded in `README.md`'s "Detector Training Metrics" section). Added 2026-09-07. |
| `surf_count_model_calibration/` | Calibration/coverage check for the surf-count GBT quantile model (not the detector) — reuses `fit_surfer_count_model.py`'s own `load_and_prepare()`/`train_test_split`/`standardize()`/`fit_quantile_model_robust()` pipeline to measure real empirical coverage on a held-out test split at 4 nominal intervals (20/40/60/80%). `plot_calibration.py` writes `calibration_plot.png` (git-tracked, embedded in `README.md`'s "Model Calibration" section). Re-verification found the 80% interval's real coverage today (~83%) contradicts an earlier-documented ~65-67% figure — see `PROJECT_HISTORY.md`'s 2026-09-07 entry for the investigation (real y_test zero-inflation, not a bug). Added 2026-09-07. |
...
| `data/cvat_out_dataset_2025_09_24_coco 1.0.zip` | Earlier zipped CVAT COCO export, superseded by the unzipped `data/cvat_out_coco/` — verified 2026-09-05 to have the exact same 57 images (byte-identical filename list) as `data/cvat_out_coco/splits/`, just pre-split (one flat `instances_default.json` instead of train/val/test). Genuinely redundant with what's in `data/`. |
| `data/cvat_out_yolo_detect_1.0.zip` | Earlier zipped CVAT YOLO export, superseded by the unzipped `data/cvat_out_yolo_rebuilt/` — also untiled (this predates the tiling step) and, like all YOLO-format label files, structurally cannot carry per-box attributes (confirmed 2026-09-05: a real label line is just `class x_center y_center width height`, no attribute field at all) — not usable for the posture-attribute labeling work regardless of redundancy. |
